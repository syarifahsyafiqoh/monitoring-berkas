...
<head>
    ...
    <title>Rekap Total Anggaran per Modul - BPDAS Barito</title>
    <style>
        body {
...
<body>
    ...
    <h1>Rekap Total Anggaran / Biaya per Modul</h1>

    <?php if (!empty($rekap_biaya)): ?>
        <table>
            <thead>
                <tr>
                    <th>Modul</th>
            ...
            <tbody>
                ...
                <?php foreach ($rekap_biaya as $r): ?>
                    ...
                    <tr>
                        ...
                        <td class="right">Rp <?= number_format($total_modul, 0, ',', '.') ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                <tr class="total-row">
                    <td class="right">Total Keseluruhan</td>
                    <td class="right">Rp <?= number_format($grand_total, 0, ',', '.') ?></td>
                </tr>
            </tfoot>
        </table>
    <?php else: ?>
        <p style="text-align: center; font-style: italic;">Belum ada data biaya/anggaran untuk operator ini.</p>
    <?php endif; ?>
