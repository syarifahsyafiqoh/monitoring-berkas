...
<head>
    <meta charset="UTF-8">
    <title>Laporan Berkas Terverifikasi - BPDAS Barito</title>
    <style>
        body {
...
<body>
    ...
    <h1>Laporan Berkas Terverifikasi</h1>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th width="140">No Berkas</th>
                    <th>Modul</th>
                    <th>Operator</th>
                    <th width="120">Status</th>
                    <th class="center" width="140">Tanggal Input</th>
                </tr>
            </thead>
            <tbody>
                <?php $no = 1; foreach ($berkas_terverifikasi as $b): ?>
                    <tr>
                        <td class="center"><?= $no++ ?></td>
                        <td><strong><?= esc($b['no_berkas']) ?></strong></td>
                        <td><?= ucwords(str_replace('_', ' ', esc($b['jenis_modul']))) ?></td>
                        <td><?= esc($b['operator_name'] ?? '-') ?></td>
                        <td class="center"><span class="badge-success">Diverifikasi</span></td>
                        <td class="center"><?= date('d M Y H:i', strtotime($b['created_at'])) ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    <?php else: ?>
        <p style="text-align: center; font-style: italic; margin-top: 20px;">Tidak ada berkas terverifikasi untuk ditampilkan.</p>
    <?php endif; ?>
