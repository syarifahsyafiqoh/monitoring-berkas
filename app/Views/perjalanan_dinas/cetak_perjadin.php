...
<head>
    <meta charset="UTF-8">
    <title>Laporan Perjalanan Dinas - BPDAS Barito</title>
    <style>
        body {
        ...
        th, td {
            border: 1px solid #000 !important; /* garis tebal semua sisi */
...
<body>
    ...
    <?php if (!empty($perjalanan_dinas)): ?>
        <table>
            <thead>
                <tr>
                    <th class="center" width="40">No</th>
                    <th width="140">No Surat Tugas</th>
                    <th width="280">Uraian Tugas</th>
                    <th width="110">No Akun</th>
                    <th class="right" width="80">Harian</th>
                    <th class="right" width="80">Penginapan</th>
                    <th class="right" width="80">Transport</th>
                    <th class="right" width="100">Total Biaya</th>
                    <th width="60">Seksi</th>
                    <th class="center" width="110">Tanggal Input</th>
                </tr>
            </thead>
            <tbody>
                <?php $no = 1; foreach ($perjalanan_dinas as $row): ?>
                    <?php
                    $total_biaya = ($row['harian_perjalanan'] ?? 0) + ($row['penginapan'] ?? 0) + ($row['transport'] ?? 0);
                    ?>
                    <tr>
                        <td class="center"><?= $no++ ?></td>
                        <td><?= esc($row['no_surat_tugas'] ?? '-') ?></td>
                        <td class="uraian"><?= esc($row['uraian'] ?? '-') ?></td>
                        <td><?= esc($row['no_akun'] ?? '-') ?></td>
                        <td class="right">Rp <?= number_format($row['harian_perjalanan'] ?? 0, 0, ',', '.') ?></td>
                        <td class="right">Rp <?= number_format($row['penginapan'] ?? 0, 0, ',', '.') ?></td>
                        <td class="right">Rp <?= number_format($row['transport'] ?? 0, 0, ',', '.') ?></td>
                        <td class="right fw-bold">Rp <?= number_format($total_biaya, 0, ',', '.') ?></td>
                        <td><?= esc($row['seksi'] ?? '-') ?></td>
                        <td class="center"><?= $row['created_at'] ? date('d/m/Y H:i', strtotime($row['created_at'])) : '-' ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                <tr class="total-row">
                    <td colspan="7" class="right">Total Keseluruhan</td>
                    <td class="right">Rp <?= number_format(array_sum(array_map(function($r) { return ($r['harian_perjalanan'] ?? 0) + ($r['penginapan'] ?? 0) + ($r['transport'] ?? 0); }, $perjalanan_dinas)), 0, ',', '.') ?></td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
        </table>
    <?php else: ?>
        <p style="text-align: center; font-style: italic; margin-top: 30px;">Belum ada data perjalanan dinas.</p>
    <?php endif; ?>
